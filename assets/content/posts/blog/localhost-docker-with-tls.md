...
The project is an attempt to make the development setup of a microservice architecture easier. Collections of custom Docker images and segmented Docker Compose files can re-create any server environment locally. Instead of every project maintaining individual docker-compose files or deployment details, there is a way to structure like-environments to reuse shared building blocks that can be combined for the project you are actually working on.
...
Project-specific Compose files then layer on top of it with the details that vary from project to project, usually bind mounts and dependency-specific volumes. That gives each project a smaller, clearer setup file. Developers can commit a Compose file for their project without duplicating the entire environment definition every time. Furthermore, changes to the base compose or images will eventually propagate to other developers without the use of an image registry.
...
The database service is the only service with volumes configured in the base file. These volumes mount `../database/data` to persist data between database teardowns and `../database/initdb/` to bootstrap the database with valid SQL scripts on first initialization (or when `../database/data` is empty and the container is restarted).
...
- isolating project-specific volumes to the project that needs them
